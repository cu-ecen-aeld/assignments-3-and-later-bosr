...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,
  ...
  if ((thread == NULL) || (mutex == NULL)) {
    perror("Pointer to thread or mutex is null");
    return false;
  } else if ((wait_to_obtain_ms < 0) || (wait_to_release_ms < 0)) {
    ERROR_LOG("wait_to_obtain_ms or wait_to_release_ms should be positive");
    return false;
  }

  struct thread_data *thread_func_args = malloc(sizeof(struct thread_data));
  thread_func_args->mutex = mutex;
  thread_func_args->wait_to_obtain_ms = (unsigned int)wait_to_obtain_ms;
  thread_func_args->wait_to_release_ms = (unsigned int)wait_to_release_ms;
  thread_func_args->thread_complete_success = false;
  ...
  DEBUG_LOG("successfully started thread");
